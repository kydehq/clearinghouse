...
from .audit import get_audit_payload  # bleibt verfügbar, wird hier nicht genutzt
...
class PocDemoPayload(BaseModel):
    ...
    use_case: Optional[str] = "mobility"
...
def _fare_for(mode: str, rng: random.Random) -> float:
    ...
    roll = rng.random()
    if roll < 0.6: return round(rng.uniform(1.0, 5.0), 2)
    if roll < 0.9: return round(rng.uniform(8.0, 30.0), 2)
...
def _simulate_energy(policy: Dict[str, Any], tx_count: int):
    ...
    pv_price      = float(prices.get("pv_eur_per_kwh")      or 0.14)
    local_price   = float(prices.get("local_eur_per_kwh")   or 0.18)
    ...
    for r in rules:
        m = (r.get("match") or {})
        if m.get("role") == "all" and r.get("round"): round_mode = r["round"]
        if m.get("role") in ("fleet_partner","operator"):
